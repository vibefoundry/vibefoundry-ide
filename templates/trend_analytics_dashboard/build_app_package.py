"""
Build the trend_analytics_dashboard PWA into a Track 2 distributable
package at output_folder/trend_analytics_dashboard/.

Plain-script architecture — no Vite, no npm. Re-uses the dev-asset
prep flow (downloads UMDs, stages data), then copies the src_app/
tree as-is into the package's application_files/ directory. The
package's launcher trio (pc_start.bat, mac_start.command, mac_start.sh)
serves application_files/ via a static HTTP server.
"""
import os
import shutil
import sys
...
python3 - <<'PY'
import http.server, socket, socketserver, threading, webbrowser
