"""Flask backend for data_chatbot — the code-gen chatbot over 3YR_Data.parquet.
...
exposed over HTTP. It never does data work itself — it calls into the modules:
  ...
  prompt    -> build prompts + call OpenAI (classify, codegen, final answer)
...
app_folder/scripts/data_chatbot/), deliberately departing from the project's
...
# backend/ -> data_chatbot/ — everything the app needs (data/, output/,
...
if __name__ == "__main__":
    ...
    print(f"[data_chatbot] backend on http://localhost:{port}")
    table_names = list(metadata_mod.get_tables().keys())
    print(f"[data_chatbot] tables: {table_names or '(none — awaiting upload)'}")
    print(f"[data_chatbot] output: {TASK_OUTPUT}")
