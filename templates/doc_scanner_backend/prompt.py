...
the allowed per-item `item_type` list). `invoke_openai` base64-encodes a receipt
image and sends it, alongside the prompt, to the OpenAI vision API — asking for
a JSON object that matches output_schema.json.
"""
...
def build_prompt(template: str = "default") -> str:
    ...
    item-type list is pulled straight from the schema's per-item `item_type`
    enum so the prompt and the schema can never drift apart."""
    ...
    item_type_block = "\n".join(f"- {c}" for c in item_types) or "(none defined)"

    return (
        f"{tpl['system']}\n\n"
        f"=== FIELD RULES ===\n{rules}\n\n"
        f"=== ALLOWED ITEM TYPES (pick exactly one per line item) ===\n{item_type_block}\n\n"
