...
- **Do not run setup or applications yourself to "test" your work.** When you finish writing or changing an app, do NOT launch it, start its server, or run its `setup.*` / `run_app.*` scripts to check whether it works.
- **The user runs it locally themselves.** Ask the user to run the launcher for their operating system — double-click `run_app.bat` (Windows) or `run_app.command` / `run_app.sh` (Mac) — and report back what they see. That is how the app gets verified.
- **The only exception:** if the user **explicitly** asks you to run the setup or the app, then run it.
- **Do not spawn extra commands just to double-check your own output** — prompt the user to run the launcher and read the result instead.
