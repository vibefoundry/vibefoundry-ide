...
const MAC_SETUP = [
  ...
  'echo "[1/4] Checking Python and Miniconda..."',
  ...
  'echo "[3/4] Installing VibeFoundry and Python data libraries..."',
  'python -m pip install matplotlib plotly pandas numpy && python -m pip install -U vibefoundry',
  'echo "[4/4] Verifying VibeFoundry and registering the pane..."',
...
const WINDOWS_SETUP = [
  ...
  'Write-Host "[1/4] Checking Python and Miniconda..."',
  ...
  'Write-Host "[2/4] Installing Node.js and Git..."',
  '& "$env:USERPROFILE\\miniconda3\\Scripts\\conda.exe" install -y nodejs git',
  'Write-Host "[3/4] Installing VibeFoundry and Python data libraries..."',
  ...
  'Write-Host "[4/4] Verifying VibeFoundry and registering the pane..."',
...
function buildSetupInstructions(os) {
  ...
  return [
    ...
    commands,
    "",
    "When all four steps succeed, restart the Codex desktop app if needed, scaffold the current task root, and open VibeFoundry.",
  ].join("\n");
}
