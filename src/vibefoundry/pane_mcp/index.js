...
//  - plugin cache: index.js runs from a plugin dir, nowhere near a python env —
//    but setup always installs into ~/miniconda3, so probe that fixed location
//    directly. Critical on Windows, where the shell fallback runs cmd.exe /c
//    with the app's inherited PATH (no login shell, so a post-launch conda
//    install is invisible and `vibefoundry` never resolves).
