...
The `_app.py` output folder is named after the **task folder** (not the `_app.py` file). Step outputs get subfolders named after their `.py` file. When a step runs individually, it writes into its subfolder inside the task's output folder (same place as when orchestrated).
...
- **No timestamps** — each run overwrites the previous output
- **Output folder naming**: `_app.py` uses the task folder name (e.g., `regional_analysis_app.py` → `output_folder/regional_analysis/`). Steps get subfolders named after their `.py` file (e.g., `step1_clean.py` → `output_folder/regional_analysis/step1_clean/`)
