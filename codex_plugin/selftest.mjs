...
const MCP = process.argv[2] || join(HERE, "plugins/vibefoundry/server/index.js");
...
check("tools/list exposes all five", [
  "open_vibefoundry",
  "vf_request",
  "vf_catalog",
  "scaffold_project",
...
const setup = await call("tools/call", {
  name: "setup_vibefoundry",
  arguments: { os: "mac" },
});
check("setup tool returns local runtime commands",
  ok(setup) &&
    setup.result.structuredContent.os === "mac" &&
    setup.result.content[0].text.includes("pip install -U vibefoundry"),
  ok(setup) ? setup.result.structuredContent.os : why(setup));

const scaffold = await call("tools/call", {
  name: "scaffold_project",
  arguments: { projectRoot: PROJECT, project_name: "selftest" },
});
let scaffoldFilesExist = false;
let agentsIsCanonical = false;
try {
  await Promise.all([
    access(join(PROJECT, "app_folder/scripts")),
    access(join(PROJECT, "app_folder/meta_data")),
    access(join(PROJECT, "input_folder")),
    access(join(PROJECT, "output_folder")),
  ]);
  scaffoldFilesExist = true;
  const agents = await readFile(join(PROJECT, "AGENTS.md"), "utf8");
  agentsIsCanonical = agents.length > 1000 && agents.includes("VibeFoundry");
} catch {}
check("scaffold tool writes the bundled project structure",
  ok(scaffold) && scaffoldFilesExist && agentsIsCanonical,
  ok(scaffold) ? JSON.stringify(scaffold.result.structuredContent) : why(scaffold));
